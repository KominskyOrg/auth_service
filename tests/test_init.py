...
def test_create_app_development(
    ...
    assert app.config['ENV'] == 'development', f"Expected ENV to be 'development', got '{app.config['ENV']}'."

    # Ensure that init_db was called with app
    mock_init_db.assert_called_once_with(app)

    # Ensure that register_blueprint was called with auth_service_bp
    mock_register_blueprint.assert_any_call(auth_service_bp)
    ...
    mock_register_blueprint.assert_any_call('swaggerui_blueprint_mock', url_prefix='/api/docs')

    # Ensure logging setup (StreamHandler)
    assert any(isinstance(h, logging.StreamHandler) for h in app.logger.handlers), "StreamHandler not found in logger handlers."
    ...
    for log_msg in expected_logs:
        assert log_msg in caplog.text, f"Expected log message '{log_msg}' not found in caplog."
...
def test_create_app_production(
    ...
    assert app is not None, "App instance should not be None."
    assert app.config['ENV'] == 'production', f"Expected ENV to be 'production', got '{app.config['ENV']}'."
    ...
    assert len(file_handlers) == 1, "Expected exactly one FileHandler in logger handlers."
    ...
    assert actual_log_filename == expected_log_filename, (
    ...
    assert 'Swagger UI has been registered' not in caplog.text, "Swagger UI should not be registered in production."

def test_register_swagger_ui_import_error(
    set_development_env,
    mock_init_db,
    caplog,
    mock_flask_swagger_ui_import_error
):
    ...
    expected_log_message = 'Flask-Swagger-UI is not installed. Swagger UI not available.'
    assert expected_log_message in caplog.text, f"Expected log message '{expected_log_message}' not found in caplog."
...
def test_register_swagger_ui_exception(
    ...
    assert 'Swagger UI has been registered at /api/docs.' not in caplog.text, "Swagger UI should not be registered after exception."
